...
Dev store API, es una API para gestionar una pequeña tienda con productos y enlaces de tiendas externas, partiendo la estructura siguiente
 ...
 - **Suppliers**
Suministradores, diseñadores del producto

 - **Stores**
Tiendas donde se pueden encontrar los productos
...
Productos disponibles
 ...
 - **LinksProducts**
Enlaces externos de las tiendas por cada producto

 - **Collections**
Colecciones, Promociones de productos

 **Observaciones**
 
- Las categorías tienen un campo `active` para activar o desactivar los productos de esa categoría.
...
Editar el archivo `app\settings.php` con los datos de conexión al servidor SQL.
...
`GET devstore/public/v1/categories` Obtener listado de las las categorías.

`GET devstore/public/v1/suppliers` Obtener todos los suministradores de productos.
...
Obtener todos los productos disponibles, solamente los campos esenciales para mostrar un listado rápido.
...
También se puede usar parámetros de consulta para filtrar y obtener colecciones de recursos, usando el parámetro  `filter` especificando el nombre del campo, el operador y el valor `?filter=field:operator:[value]`, por ejemplo:

`GET devstore/public/v1/categories?filter=active:eq[1]` Devolverá listado de las categorías activas.
...
## Ordenamiento
La colección de datos se puede ordenar por un campo o varios campos de modo ascendente o descendente, para indicar se usa el parámetro `sort_by`
...
`GET devstore/public/v1/products?limit=10`Ppara obtener 10 productos.
...
`GET devstore/public/v1/categories?filter=active:eq[1]` Obtener todas las categorías  que están disponibles, campo `active` debe ser 1
...
Obtener todos los productos disponibles de una categoría mediante el ID de categoría.
